    ...
    <div class="admin-panel flex-1">
      ...
      <div class="panel-body">
        <ul class="list-none-style" v-if="this.type==0">
          ...
          <li>证件类型: <span v-if="messages.realname.idType==0">身份证</span><span
            v-if="messages.realname.idType==1">护照</span></li>
      ...
      <div class="panel-body">
        <ul class="list-none-style">
          ...
          <li v-if="messages.realname.status==-1 || messages.realname.status==-2">
            不通过原因:
            <span
              v-if="messages.realname.status==-1 || messages.realname.status==-2">{{messages.realname.reason}}</span>
          </li>
        </ul>
      </div>
    </div>
                ...
                <div class="">
                  ...
                  <div class="title-reason inline-block text-align-c ">
                    <div class="inline-block ">
                      ...
                      <span
                        v-if="(message.status==-1 || message.status==-2) && message.reason!=null && message.reason!='' "
                        class="text-danger">({{message.reason}})</span>
                    </div>
                    <div class="inline-block float-r"><span @click="showDetail($index)" class="cursor"><i
                      class="icon iconfont icon-oc-dropdown"></i></span></div>
                  </div>
                </div>
